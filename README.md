...
## Контрольные вопросы:
	1.	Контейнер — это изолированная среда, в которой можно запускать приложение вместе со всеми его зависимостями. Он использует ядро операционной системы хоста и работает как отдельный процесс. В отличие от виртуальной машины, контейнер не включает в себя отдельную операционную систему, а использует ресурсы хостовой системы, благодаря чему запускается быстрее и потребляет меньше ресурсов.
	2.	Dockerfile — это текстовый файл, содержащий инструкции для создания Docker-образа. В нем описывается, на каком базовом образе строится приложение, какие зависимости необходимо установить, какие файлы нужно скопировать в образ, и какая команда должна быть выполнена при запуске контейнера. Dockerfile позволяет автоматизировать процесс сборки среды выполнения приложения.
	3.	Команда docker build считывает инструкции из Dockerfile и создает на их основе Docker-образ. При выполнении этой команды Docker пошагово обрабатывает все указанные действия и формирует финальный образ, который можно использовать для запуска контейнера.
	4.	Образ (image) — это шаблон, содержащий все необходимые файлы, зависимости и инструкции для запуска приложения. Контейнер — это запущенный экземпляр образа. Образ является статичным, а контейнер — это активный процесс, который выполняется на основе этого образа.
	5.	Чтобы удалить все неиспользуемые контейнеры и образы в Docker, используется команда docker system prune -a. Она удаляет остановленные контейнеры, неиспользуемые образы, сети и кэш сборки, освобождая место на диске. Следует быть осторожным при использовании этой команды, так как она может удалить важные данные, если они больше не используются в активных контейнерах.
	6.	Если изменить содержимое файла main.py и заново выполнить команду docker build, Docker пересоберёт только те слои образа, которые были изменены, и все слои после них. Это значит, что если в Dockerfile есть инструкция COPY main.py, то после изменения этого файла кэш на этом шаге станет недействительным, и все последующие шаги будут пересобраны заново. Это позволяет избежать полной пересборки образа, если изменения касаются только части приложения.
